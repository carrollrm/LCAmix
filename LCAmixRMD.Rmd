...
As our first step of analysis, we will examine the correlation matrix of the chemicals. This is done with the base cor() function as well as with the corrplot package in R. From these results, we can see that $-0.04 \lt \rho \lt 0.67$. The chemical pairs with the highest amount of correlation include $\rho_{m1,m2}=0.41$, $\rho_{m5,m8}=0.58$, and $\rho_{m7,m9}=0.67}$. This indicates little to moderate correlation in the chemicals presented here. 
...
The plot produced from this code displays proportion of above the median measures for each class (classes 1-3 left to right) and chemical ($p_{km}$ from the manuscript, the Pr(2) column from the console output). In my opinion, this is not the best way to display these results. 
